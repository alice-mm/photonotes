...
[TIP]
====
Maintenant, j’active parfois l’option https://fr.wikipedia.org/wiki/Touche_r%C3%A9manente[« Touches rémanentes » (_« Sticky keys »_)] de mon ordi, comme ça j’appuie juste une fois sur kbd:[Ctrl] avant chaque clic au lieu de maintenir la touche.
Ça aide déjà pas mal.
====
...
* Si je ne vois pas trop quelles zones sont affectées par l’un des _sliders_, je lui fous temporairement des valeurs abusées juste pour me faire une idée.

* J’essaye de voir avec par exemple{nbsp}-10 et{nbsp}10 si je veux plutôt partir dans le positif ou le négatif (ou rester à zéro), et enfin je recherche une « vraie » valeur.
